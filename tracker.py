...
def mask2polygon(n_detect, all_masks):
    # all_masks is of type torch.Tensor, whose values are boolean (True, False), should be converted to polygons
    all_polygons = []
    if n_detect != 0:
        for pred_mask in all_masks:
            # Convert pred_mask to a 8-bit numpy array, which can then be used to find contours
            mask = pred_mask.cpu().numpy().astype('uint8')
            contour, _ = cv2.findContours(mask, cv2.RETR_LIST, cv2.CHAIN_APPROX_NONE)
            contour = np.array([[c[0][0], c[0][1]] for c in contour[0]])
            all_polygons.append(contour)
    return len(all_polygons), np.array(all_polygons) 

''' Calculate inter_area/box_area_1 '''
def calculate_intersection(xmin1,xmax1,ymin1,ymax1,xmin2,xmax2,ymin2,ymax2):
    if xmin1>xmax2 or xmax1<xmin2 or ymin1>ymax2 or ymax1<ymin2:   
        ...
        intersection = inter_area / box_area_1
...
def detect(video, subset, config, weight_path, label):
    ...
    cfg_detect.MODEL.ROI_HEADS.SCORE_THRESH_TEST = 0.6
    ...
    ''' initialize KLT: reset global variables for KLT algorithm '''
    KLT_Mask.correct_n = 0 
    KLT_Mask.pred_n_lastframe = 0 
    KLT_Mask.appear_frame = 0 
    KLT_Mask.KLT_features = [] 
    KLT_Mask.Images = [] 
    KLT_Mask.Scores = [] 
    KLT_Mask.Classes = [] 
    KLT_Mask.Polygons = [] 
    ...
    while (cap.isOpened()):
        ...
        if ret == True:
            ...
            all_masks = outputs['instances'].__dict__['_fields']['pred_masks'].tolist()
            ...
            ''' convert binary mask to polygon'''
            pred_n, all_polygons = mask2polygon(len(all_boxes), torch.tensor(all_masks))

            ''' KLT '''
            n, all_scores, all_classes, all_boxes, image = KLT(pred_n, all_scores, all_classes, all_boxes, all_polygons, image)
                ...
                for i, det in enumerate(pred):
                    ...
                    if len(bbox_ids) != 0:
                        for bbox_id in bbox_ids:
                            ...
                            cv2.putText(image,category,(xmin+10,ymin+20),cv2.FONT_HERSHEY_SIMPLEX,0.65,(225,0,0),2)
                            cv2.putText(image,'ID-'+ID,(xmin+10,ymin+40),cv2.FONT_HERSHEY_SIMPLEX,0.65,(225,0,0),2)
